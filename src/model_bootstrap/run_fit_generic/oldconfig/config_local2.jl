...
#FIT configuration
    ...
    function iniparam()
        ...
        initialparameters = Matrix{Float64}(undef, 11,7)
        initialparameters[1,:] .= paratemp[idxs[1],:]
        ub = min.([SRange[k][1] for k in eachindex(SRange)],paratemp[idxs[1],:] .+ .2 .* paratemp[idxs[1],:])
        lb = max.([SRange[k][1] for k in eachindex(SRange)],paratemp[idxs[1],:] .- .2 .* paratemp[idxs[1],:])
        db = ub .-lb
        for i=1:10
            initialparameters[i+1,:] .= lb .+ rand(7) .* db
        end
...
#=         for i =2:5
    ...
    maxoptimtime =1000#2000
    maxoptimiters =1000#2000
    nboptims = 1
    autodiff = true
