...
"""
function optim_function(SRange, FRange, optim_struct::OptimStruct, args...; maxrnaLC = 10, maxrnaFC = 60, freeparametersidx =[x for x in eachindex(SRange)], fixedparameters =[-1], multipledistribution=false, multipledistributiontypes=122, kwargs...)
    @unpack data, dist, model, AutoDiff = optim_struct
        if data.burstsinglet == :with
            if AutoDiff
                err_func = ini_optimAD(optim_struct)
            else
                err_func = ini_optim(optim_struct)
            end
        elseif data.burstsinglet == :without
            if AutoDiff
                err_func = ini_optim_withoutsingletAD(optim_struct)
            else
                err_func = ini_optim_withoutsinglet(optim_struct)
            end
        end
...
function ini_data(optim_struct::OptimStruct, FRange; kwargs...)
    ...
    for i in eachindex(FRange)
        if (FRange[i][2]>0)
            ...
            if minimum(datatemp[2])<=0
                @warn "There are survival probabilities <= zero"
            end
        else
    ...
    return StoThyLiveCell.DataFit{typeof(data.datatypes),typeof(data.data)}(data.datatypes, Tuple(datafit), data.detectionLimitLC, data.detectionLimitNS, data.burstsinglet)
...
function ini_optimAD(optim_struct::OptimStruct; kwargs...)
...
function ini_optim_withoutsinglet(optim_struct::OptimStruct; kwargs...)
    function err_func(params,optim_struct_wrapper::OptimStructWrapper)
        parameters = utiles.mergeparameter_base(optim_struct_wrapper.fixedparam, params, optim_struct_wrapper.freeparametersidx)
        #@unpack utileMat = optim_struct_wrapper
        #model outputs
...
function ini_optim_withoutsingletAD(optim_struct::OptimStruct; kwargs...)
    function err_func(params::AbstractVector{T},optim_struct_wrapper::OptimStructWrapper) where T
        @unpack utileMat = optim_struct_wrapper
        parameters = utiles.mergeparameter_base(optim_struct_wrapper.fixedparam, params, optim_struct_wrapper.freeparametersidx)
